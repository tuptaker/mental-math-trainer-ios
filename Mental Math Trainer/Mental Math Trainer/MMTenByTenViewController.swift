...
class MMTenByTenViewController: MMBaseProblemViewController, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        if let multTableCell = cell as? MMMultTableCollectionViewCell, let _ = multTableCell.multResultLabel {
            ...
            if (indexPath.row == 0 && indexPath.section >= 1) {
                multTableCell.multResultLabel.text = "\(indexPath.section)"
                multTableCell.backgroundColor = UIColor.white
                multTableCell.multResultLabel.textColor = UIColor.black
            }
            if (indexPath.section == 0 && indexPath.row >= 1) {
                multTableCell.multResultLabel.text = "\(indexPath.row)"
                multTableCell.backgroundColor = UIColor.white
                multTableCell.multResultLabel.textColor = UIColor.black
            }
